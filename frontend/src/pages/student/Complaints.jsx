...
export default function StudentComplaintsPage() {
  ...
  const handleReopen = async (id) => {
    ...
    try {
      ...
      // Handle rate limit from backend - check for proper property
      // if (data.rateLimited || res.status === 429) {
      //   setRateLimit(true);
      //   setRateLimitMessage(data.message || "You've reached the maximum number of re submit attempts. we will resolve your issue and notify you, if you will not satisfied you can contact us at our office");
      //   alert(data.message || "You've reached the maximum number of re submit attempts.we will resolve your issue and notify you, if you will not satisfied you can contact us at our office");
      //   return;
      // }
      ...
      {/* {rateLimit && (
        <div className="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded mb-4">
          <div className="flex justify-between items-center">
            <div>
              <strong>⚠️ Limit Reached:</strong> {rateLimitMessage}
            </div>
            <button 
      ...
      {complaints.length === 0 ? (
        ...
        complaints.map((c) => (
          <div key={c._id} className="bg-white border-l-4 border-blue-500 rounded-r-lg p-5 mb-5 shadow-sm">
            ...
            {c.resolvedAction && (
              <div className="mt-2 p-2 border rounded bg-gray-50">
                ...
                <p className="text-xs text-orange-600 mt-2">
                   re solved {c.reSolvedCount} time(s)
                </p>
              </div>
            )}
            ...
            {c.reOpenedCount >= 0 && (
              <div>
                <span className="text-gray-500">Re submited: </span>
                <span className="font-medium text-orange-600">
                  {c.reOpenedCount} time(s)
                  {/* {c.reOpenedCount === 0 ? "(1 attempt remaining)" : "(🚫 Maximum  reached. we will resolve and notify you .)"} */}
                </span>
              </div>
            )}
  ...
  <div className="mt-4 flex flex-col gap-3">
    ...
    >
      ✅ Click here to confirm if you are satisfied!
    </button>
    ...
    {selectedComplaint === c._id ? (
      <div className="mt-2 p-3 bg-gray-50 rounded-lg">
        ...
        <div className="flex gap-2">
          <button
            onClick={() => handleReopen(c._id)}
            className="px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700 disabled:opacity-50 transition-colors flex-1"
      ...
      <button
        ...
        disabled={c.reOpenedCount >= 1}
      >
        {c.reOpenedCount >= 1 ? (
            c.reSolvedCount >=2 ? (
              <span>
            🚫  we have re solved your complaint.If you are not satisfied, contact us at our office.
          </span>
            ) : (
            <span>
            🚫 Maximum submit attempts reached. we will resolve and notify you .
          </span>
            )
        ) : (
          <span>
            ❓ Not satisfied? Click here to explain why and we'll resolve it.
          </span>
        )}
      </button>
    )}
      ...
      <div className="text-xs text-gray-500 mt-1">
        {c.reOpenedCount >= 0 && (
          <span className="text-orange-600 ml-2">
            Re opened {c.reOpenedCount} time(s)
            {c.reOpenedCount === 0 ? "(1 attempt remaining)" : "(Maximum reached)"}
          </span>
        )}
      </div>
    )}
